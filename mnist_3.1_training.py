...
for i in range(1000):
  ...
  if i%100 == 0:
    train_accuracy = sess.run(accuracy, feed_dict={X: batch[0], Y_: batch[1], pkeep: 1.0})
    print("step %d, training accuracy %g"%(i, train_accuracy))

  sess.run(train_step, feed_dict={X: batch[0], Y_: batch[1], lr: learning_rate, pkeep: 0.75})
  
a = sess.run(accuracy, feed_dict={X: mnist.test.images, Y_: mnist.test.labels, pkeep: 1.0})
print("test accuracy %g"%a)
